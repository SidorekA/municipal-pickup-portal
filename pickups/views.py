...
def api_get_pickup_dates(request, location_id):
    ...
    # Security Fix: Check authentication manually to avoid 302 redirect for JSON API
    if not request.user.is_authenticated:
        return JsonResponse({'error': 'Brak uwierzytelnienia'}, status=403)
    ...
    # Security Fix: Check authorization to prevent IDOR on location_id
